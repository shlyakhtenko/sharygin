...
> Let the vertex of an angle be located outside a circle, and let the sides of the angle intersect
> Let the vertex of an angle lie inside a circle. Prove that the angle is measured by the half-sum
> of the arcs one of which is enclosed between its sides and the other between their extensions.
-/

namespace Soultions.Sharygin.Page11.Problem5

open Euclid Plane
open Soultions.Sharygin.Page11.Problem5.Tarski
open Soultions.Sharygin.Page11.Problem5.Midpoint
...
private theorem triangle_turn (M : AngleMeasurement G) [M.Axioms]
  ...
  obtain ⟨copies, hyax⟩ :=
    Soultions.Sharygin.Page11.Problem5.Affine.triangleCopies_aligned G hcol
...
omit [G.Axioms] in
private theorem vertical_angle_measure (M : AngleMeasurement G) [M.Axioms]
    {a b c d v : G.Point} (sense : RotationSense)
    (hcv : c ≠ v) (hav : a ≠ v) (hdv : d ≠ v) (hbv : b ≠ v)
    (hcva : G.Bet c v a) (hdvb : G.Bet d v b) :
    M.measure ⟨a, v, b, sense⟩ = M.measure ⟨c, v, d, sense⟩ := by
  calc
    M.measure ⟨a, v, b, sense⟩ =
        M.add
          (M.measure ⟨a, v, d, sense⟩)
          (M.measure ⟨d, v, b, sense⟩) :=
      AngleMeasurement.Axioms.measure_add a d b v sense hav hdv hbv
    _ = M.add (M.measure ⟨a, v, d, sense⟩) M.halfTurn := by
      rw [AngleMeasurement.Axioms.measure_straight d v b sense hdv hbv hdvb]
    _ = M.add M.halfTurn (M.measure ⟨a, v, d, sense⟩) :=
      AngleMeasurement.Axioms.add_comm _ _
    _ = M.add
          (M.measure ⟨c, v, a, sense⟩)
          (M.measure ⟨a, v, d, sense⟩) := by
      rw [AngleMeasurement.Axioms.measure_straight c v a sense hcv hav hcva]
    _ = M.measure ⟨c, v, d, sense⟩ :=
      (AngleMeasurement.Axioms.measure_add c a d v sense hcv hav hdv).symm

private theorem interior_chord_angle (M : AngleMeasurement G) [M.Axioms]
    {circle : Circle G} (config : InteriorChordConfiguration G circle) :
    M.twice
        (M.measure
          ⟨config.enclosedArc.start, config.vertex, config.enclosedArc.finish,
            config.enclosedArc.sense⟩) =
      M.add (config.enclosedArc.measure M) (config.extensionArc.measure M) := by
  let a := config.enclosedArc.start
  let b := config.enclosedArc.finish
  let c := config.extensionArc.start
  let d := config.extensionArc.finish
  let v := config.vertex
  let sense := config.enclosedArc.sense
  have hva : v ≠ a :=
    inside_point_ne_onCircle G config.vertex_inside config.enclosedArc.start_onCircle
  have hvb : v ≠ b :=
    inside_point_ne_onCircle G config.vertex_inside config.enclosedArc.finish_onCircle
  have hvc : v ≠ c :=
    inside_point_ne_onCircle G config.vertex_inside config.extensionArc.start_onCircle
  have hvd : v ≠ d :=
    inside_point_ne_onCircle G config.vertex_inside config.extensionArc.finish_onCircle
  have hca : c ≠ a := by
    intro hca
    have hcva : G.Bet c v a := config.first_chord
    rw [hca] at hcva
    have hav : a = v :=
      Plane.Axioms.betweennessIdentity a v hcva
    exact hva hav.symm
  have hdb : d ≠ b := by
    intro hdb
    have hdvb : G.Bet d v b := config.second_chord
    rw [hdb] at hdvb
    have hbv : b = v :=
      Plane.Axioms.betweennessIdentity b v hdvb
    exact hvb hbv.symm
  have had : a ≠ d :=
    config.enclosed_start_ne_extension_finish
  have hturn_ca :
      TriangleTurn G M circle.center c a sense :=
    triangle_turn G M sense
      (center_ne_onCircle G config.extensionArc.start_onCircle)
      (center_ne_onCircle G config.enclosedArc.start_onCircle)
      hca
  have hturn_ad :
      TriangleTurn G M circle.center a d sense :=
    triangle_turn G M sense
      (center_ne_onCircle G config.enclosedArc.start_onCircle)
      (center_ne_onCircle G config.extensionArc.finish_onCircle)
      had
  have hturn_db :
      TriangleTurn G M circle.center d b sense :=
    triangle_turn G M sense
      (center_ne_onCircle G config.extensionArc.finish_onCircle)
      (center_ne_onCircle G config.enclosedArc.finish_onCircle)
      hdb
  have hturn_avd :
      TriangleTurn G M a v d sense :=
    triangle_turn G M sense hva.symm had hvd
  let extensionInscribed := M.measure ⟨c, a, d, sense⟩
  let enclosedInscribed := M.measure ⟨a, d, b, sense⟩
  have hextension :
      M.twice extensionInscribed = config.extensionArc.measure M := by
    change
      M.twice
          (M.measure
            ⟨config.extensionArc.start, config.enclosedArc.start,
              config.extensionArc.finish, config.enclosedArc.sense⟩) =
        config.extensionArc.measure M
    calc
      M.twice
          (M.measure
            ⟨config.extensionArc.start, config.enclosedArc.start,
              config.extensionArc.finish, config.enclosedArc.sense⟩) =
          M.measure
            ⟨config.extensionArc.start, circle.center,
              config.extensionArc.finish, config.enclosedArc.sense⟩ :=
        inscribed_angle_from_triangle_turn G M config.enclosedArc.sense
          config.extensionArc.start_onCircle config.extensionArc.finish_onCircle
          config.enclosedArc.start_onCircle hca had hturn_ca hturn_ad
      _ = config.extensionArc.measure M := by
        unfold Arc.measure Arc.angularMeasure
        rw [config.senses_agree]
  have henclosed :
      M.twice enclosedInscribed = config.enclosedArc.measure M := by
    exact inscribed_angle_from_triangle_turn G M sense
      config.enclosedArc.start_onCircle config.enclosedArc.finish_onCircle
      config.extensionArc.finish_onCircle had hdb hturn_ad hturn_db
  have hvertical :
      M.measure ⟨a, v, b, sense⟩ = M.measure ⟨c, v, d, sense⟩ :=
    vertical_angle_measure G M sense hvc.symm hva.symm hvd.symm hvb.symm
      config.first_chord config.second_chord
  have hexterior :
      M.measure ⟨c, v, d, sense⟩ =
        M.add (M.measure ⟨v, a, d, sense⟩) (M.measure ⟨a, d, v, sense⟩) := by
    apply exterior_angle_from_triangle_turn G M sense
      (v := a) (a := v) (b := c) (d := d)
      hva.symm hvc.symm hvd.symm had hvd had.symm hva
      (bet_symm G config.first_chord)
    exact hturn_avd
  have hfirst_ray :
      M.measure ⟨v, a, d, sense⟩ = extensionInscribed := by
    exact AngleMeasurement.Axioms.same_ray_invariant
      v c d d a sense
      (sameRay_from_far_endpoint G config.first_chord hva)
      (sameRay_refl G had.symm)
  have hsecond_ray :
      M.measure ⟨a, d, v, sense⟩ = enclosedInscribed := by
    exact AngleMeasurement.Axioms.same_ray_invariant
      a a v b d sense
      (sameRay_refl G had)
      (sameRay_from_near_endpoint G config.second_chord hvd.symm hvb)
  have hsplit :
      M.measure ⟨a, v, b, sense⟩ =
        M.add extensionInscribed enclosedInscribed := by
    calc
      M.measure ⟨a, v, b, sense⟩ =
          M.measure ⟨c, v, d, sense⟩ := hvertical
      _ = M.add
            (M.measure ⟨v, a, d, sense⟩)
            (M.measure ⟨a, d, v, sense⟩) := hexterior
      _ = M.add extensionInscribed enclosedInscribed := by
        rw [hfirst_ray, hsecond_ray]
  change
    M.twice (M.measure ⟨a, v, b, sense⟩) =
      M.add (config.enclosedArc.measure M) (config.extensionArc.measure M)
  calc
    M.twice (M.measure ⟨a, v, b, sense⟩) =
        M.twice (M.add extensionInscribed enclosedInscribed) :=
      congrArg M.twice hsplit
    _ = M.add (M.twice extensionInscribed) (M.twice enclosedInscribed) :=
      twice_add G M _ _
    _ = M.add (config.extensionArc.measure M) (config.enclosedArc.measure M) := by
      rw [hextension, henclosed]
    _ = M.add (config.enclosedArc.measure M) (config.extensionArc.measure M) :=
      AngleMeasurement.Axioms.add_comm _ _

end LocalFacts

/-- The formal statement of the intersecting-chords angle theorem. -/
...
/-- Sharygin, page 11, problem 5. -/
theorem problem5 (G : Plane) (M : AngleMeasurement G)
    [G.Axioms] [M.Axioms] :
    Statement G M := by
  intro circle config
  exact interior_chord_angle G M config

end Soultions.Sharygin.Page11.Problem5
