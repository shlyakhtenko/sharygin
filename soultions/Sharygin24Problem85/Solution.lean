import Sharygin24Problem85.Configuration

namespace Soultions.Sharygin.Page24.Problem85.Solution

open Euclid
open Soultions.Sharygin.Page24.Problem85.Configuration
...
private theorem mul_neg (x y : S.Carrier) :
...
/-- Problem 85: `KM` equals the circumradius of the equilateral triangle. -/
theorem problem85 (data : Data S) : data.km = data.circumradius := by
  apply eq_of_sub_eq_zero S
  apply mul_right_cancel S data.positive_sum
  rw [difference_of_squares S, data.division_point_calculation,
    data.circumcenter_calculation]
  unfold OrderedScalar.sub
  rw [OrderedScalar.Axioms.add_neg,
    OrderedScalar.Axioms.zero_mul]

end Soultions.Sharygin.Page24.Problem85.Solution
