...
/-!
# Problem-local midpoint grid for Sharygin, page 14, problem 20

This file repeats the affine midpoint-grid argument needed to construct the orthocenter.  It
remains local to problem 20 and does not import another problem's proof.
-/

namespace Soultions.Sharygin.Page14.Problem20.Centroid
...
/-- The local Playfair uniqueness result under the name used by this repeated midpoint proof. -/
theorem parallel_through_collinear {a x y b c : G.Point}
    (hax_parallel_bc : Parallel G a x b c)
    (hay_parallel_bc : Parallel G a y b c) :
    G.Collinear y a x :=
  parallel_through_unique G hax_parallel_bc hay_parallel_bc
...
end Soultions.Sharygin.Page14.Problem20.Centroid
