...
/-!
# Scalar identities for Sharygin, PDF page 74, problem 31

This file contains only the small amount of ordered-field algebra used by the
problem-local barycentric calculation.
-/

namespace Soultions.Sharygin.Page74.Problem31.Scalar
...
theorem sub_add_self (x : S.Carrier) :
    S.add (S.sub S.one x) x = S.one := by
  change S.add (S.add S.one (S.neg x)) x = S.one
  rw [OrderedScalar.Axioms.add_assoc,
    neg_add S, OrderedScalar.Axioms.add_zero]

theorem self_add_sub (x : S.Carrier) :
    S.add x (S.sub S.one x) = S.one := by
  rw [OrderedScalar.Axioms.add_comm, sub_add_self S]

/-- A reusable four-term reassociation, proved only from commutative addition. -/
theorem add_four (a b c d : S.Carrier) :
    S.add (S.add a b) (S.add c d) =
      S.add (S.add a c) (S.add b d) := by
  rw [OrderedScalar.Axioms.add_assoc a b,
    ← OrderedScalar.Axioms.add_assoc b c d,
    OrderedScalar.Axioms.add_comm b c,
    OrderedScalar.Axioms.add_assoc c b d,
    ← OrderedScalar.Axioms.add_assoc a c]

end Soultions.Sharygin.Page74.Problem31.Scalar
