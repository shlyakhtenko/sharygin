...
/-!
# Problem-local directed-angle layer for Sharygin, page 14, problem 22

The triangle-turn and isosceles-triangle calculations are duplicated here from the earlier
proofs, in accordance with `proof_rules.md`.
-/

namespace Soultions.Sharygin.Page14.Problem22
...
private theorem triangle_turn (M : AngleMeasurement G) [M.Axioms]
  ...
  obtain ⟨copies, hyax⟩ :=
    Soultions.Sharygin.Page14.Problem22.Affine.triangleCopies_aligned G hcol
...
/-- The locally derived triangle-angle sum, exposed for the problem 22 metric calculation. -/
...
omit [G.Axioms] in
/-- Reversing the sense and the two rays leaves the represented undirected angle unchanged. -/
theorem reverse_sense_measure_public
    (M : AngleMeasurement G) [M.Axioms]
    {a b o : G.Point} (sense : RotationSense)
    (ha : a ≠ o) (hb : b ≠ o) :
    M.measure ⟨a, o, b, sense.reverse⟩ =
      M.measure ⟨b, o, a, sense⟩ :=
  reverse_sense_measure G M sense ha hb
...
/--
The double of an inscribed angle is the corresponding directed central angle.

The two triangle-angle sums required by the direct calculation are derived locally rather than
assumed as a circle axiom.
-/
theorem inscribed_angle_twice
    (M : AngleMeasurement G) [M.Axioms]
...
/-- Problem-local compatibility name used by the independent transport layer. -/
theorem inscribed_angle
    ...
    (hrq : r ≠ q) :
    M.twice (M.measure ⟨p, r, q, sense⟩) =
      M.measure ⟨p, circle.center, q, sense⟩ :=
  inscribed_angle_twice G M sense hp hq hr hpr hrq

/--
The directed tangent--chord calculation after the metric tangent-radius fact has been reduced
to an equidistant symmetric pair on the tangent line.
-/
theorem tangent_chord_from_equidistant_reflection
    (M : AngleMeasurement G) [M.Axioms]
    {circle : Circle G} (arc : Arc G circle) {t u : G.Point}
    (htu : PointReflection G arc.start t u)
    (hot_ou : G.Congruent circle.center t circle.center u)
    (ht_off_radius : ¬G.Collinear arc.start circle.center t) :
    M.twice
        (M.measure ⟨t, arc.start, arc.finish, arc.sense⟩) =
      arc.measure M := by
  let o := circle.center
  let a := arc.start
  let b := arc.finish
  let sense := arc.sense
  have hao : a ≠ o :=
    (center_ne_onCircle G arc.start_onCircle).symm
  have hbo : b ≠ o :=
    (center_ne_onCircle G arc.finish_onCircle).symm
  have hta : t ≠ a := by
    intro hta
    subst t
    exact ht_off_radius (collinear_cyclic G (collinear_refl_left G a o))
  have hua : u ≠ a :=
    pointReflection_other_ne G htu hta
  have hab : a ≠ b := arc.start_ne_finish
  have horientation :
      G.Orientation t a o =
        (G.Orientation u a o).map RotationSense.reverse := by
    have hopposite :
        G.Orientation a o t =
          (G.Orientation a o u).map RotationSense.reverse :=
      orientation_of_pointReflection G
        (collinear_cyclic G (collinear_refl_left G a o))
        ht_off_radius htu
    rw [Plane.Axioms.orientation_cyclic a o t,
      Plane.Axioms.orientation_cyclic o t a,
      Plane.Axioms.orientation_cyclic a o u,
      Plane.Axioms.orientation_cyclic o u a] at hopposite
    exact hopposite
  have hat_au : G.Congruent a t a u :=
    congruent_symm G htu.radius
  have hangle_reflected :
      M.measure ⟨t, a, o, sense⟩ =
        M.measure ⟨u, a, o, sense.reverse⟩ := by
    exact AngleMeasurement.Axioms.sss_reversing
      t a o u a o sense
      hat_au
      (congruent_refl G a o)
      (congruent_trans G (Plane.Axioms.congruenceReversal t o)
        (congruent_trans G hot_ou
          (Plane.Axioms.congruenceReversal o u)))
      horientation
  have hangle_pair :
      M.measure ⟨t, a, o, sense⟩ =
        M.measure ⟨o, a, u, sense⟩ := by
    calc
      M.measure ⟨t, a, o, sense⟩ =
          M.measure ⟨u, a, o, sense.reverse⟩ :=
        hangle_reflected
      _ = M.measure ⟨o, a, u, sense⟩ :=
        reverse_sense_measure G M sense hua hao.symm
  have hright :
      M.twice (M.measure ⟨t, a, o, sense⟩) = M.halfTurn := by
    have hsplit :
        M.measure ⟨t, a, u, sense⟩ =
          M.add
            (M.measure ⟨t, a, o, sense⟩)
            (M.measure ⟨o, a, u, sense⟩) :=
      AngleMeasurement.Axioms.measure_add t o u a sense hta hao.symm hua
    have hstraight :
        M.measure ⟨t, a, u, sense⟩ = M.halfTurn :=
      AngleMeasurement.Axioms.measure_straight t a u sense
        hta hua htu.between
    rw [hsplit, ← hangle_pair] at hstraight
    exact hstraight
  have hoa_ob : G.Congruent o a o b :=
    circle_radii_congruent G arc.start_onCircle arc.finish_onCircle
  have hbase :
      M.measure ⟨o, a, b, sense⟩ =
        M.measure ⟨a, b, o, sense⟩ :=
    isosceles_base_angles_cyclic G M sense hoa_ob
      (center_ne_onCircle G arc.finish_onCircle) hab
  have hturn : TriangleTurn G M o a b sense :=
    triangle_turn G M sense hao.symm hbo.symm hab
  have hcentral :
      M.measure ⟨a, o, b, sense⟩ =
        M.add (M.twice (M.measure ⟨o, a, b, sense⟩)) M.halfTurn := by
    apply isosceles_turn_rearrange G M
    change
      M.add
          (M.add
          ...
          (M.measure ⟨b, o, a, sense⟩) =
        M.halfTurn at hturn
    rw [← hbase, reverse_angle_is_neg G M sense hao hbo] at hturn
    exact hturn
  have htbo :
      M.measure ⟨t, a, b, sense⟩ =
        M.add
          (M.measure ⟨t, a, o, sense⟩)
          (M.measure ⟨o, a, b, sense⟩) :=
    AngleMeasurement.Axioms.measure_add t o b a sense hta hao.symm hab.symm
  change
    M.twice (M.measure ⟨t, a, b, sense⟩) =
      M.measure ⟨a, o, b, sense⟩
  rw [htbo, twice_add G M, hright, hcentral]
  exact AngleMeasurement.Axioms.add_comm _ _

end LocalFacts

end Soultions.Sharygin.Page14.Problem22
