...
> the circle. Prove that the angle is measured by the half-difference of the arcs inside the angle
> which are cut out by its sides on the circle.
-/

namespace Soultions.Sharygin.Page11.Problem4

open Euclid Plane
open Soultions.Sharygin.Page11.Problem4.Midpoint
...
private theorem triangle_turn (M : AngleMeasurement G) [M.Axioms]
  ...
  obtain ⟨copies, hyax⟩ :=
    Soultions.Sharygin.Page11.Problem4.Affine.triangleCopies_aligned G hcol
...
omit [G.Axioms] in
private theorem exterior_angle_uses_near_points (M : AngleMeasurement G) [M.Axioms]
    {circle : Circle G} (config : ExternalSecantConfiguration G circle) :
    M.measure
        ⟨config.nearArc.start, config.vertex, config.nearArc.finish,
          config.farArc.sense⟩ =
      M.measure
        ⟨config.farArc.start, config.vertex, config.farArc.finish,
          config.farArc.sense⟩ := by
  exact AngleMeasurement.Axioms.same_ray_invariant
    config.nearArc.start config.farArc.start
    config.nearArc.finish config.farArc.finish
    config.vertex config.farArc.sense
    config.first_sameRay config.second_sameRay
...
omit [G.Axioms] in
private theorem neg_halfTurn (M : AngleMeasurement G) [M.Axioms] :
    M.neg M.halfTurn = M.halfTurn := by
  exact (eq_neg_of_add_eq_zero G M AngleMeasurement.Axioms.twice_halfTurn).symm
...
private theorem external_secant_from_triangle_turns (M : AngleMeasurement G) [M.Axioms]
    {circle : Circle G} (config : ExternalSecantConfiguration G circle)
    (h_turn_far_left :
      TriangleTurn G M circle.center config.farArc.start config.nearArc.start
        config.farArc.sense)
    (h_turn_far_right :
      TriangleTurn G M circle.center config.nearArc.start config.farArc.finish
        config.farArc.sense)
    (h_turn_near_left :
      TriangleTurn G M circle.center config.nearArc.start config.farArc.finish
        config.nearArc.sense)
    (h_turn_near_right :
      TriangleTurn G M circle.center config.farArc.finish config.nearArc.finish
        config.nearArc.sense)
    (h_turn_exterior :
      TriangleTurn G M config.vertex config.nearArc.start config.farArc.finish
        config.farArc.sense) :
    M.twice
        (M.measure
          ⟨config.farArc.start, config.vertex, config.farArc.finish,
            config.farArc.sense⟩) =
      M.sub (config.farArc.measure M) (config.nearArc.measure M) := by
  let exterior :=
    M.measure
      ⟨config.farArc.start, config.vertex, config.farArc.finish,
        config.farArc.sense⟩
  let farInscribed :=
    M.measure
      ⟨config.farArc.start, config.nearArc.start, config.farArc.finish,
        config.farArc.sense⟩
  let nearInscribed :=
    M.measure
      ⟨config.nearArc.start, config.farArc.finish, config.nearArc.finish,
        config.nearArc.sense⟩
  have hfar : M.twice farInscribed = config.farArc.measure M :=
    inscribed_angle_from_triangle_turn G M config.farArc.sense
      config.farArc.start_onCircle config.farArc.finish_onCircle
      config.nearArc.start_onCircle config.first_intersections_ne.symm
      config.start_ne_opposite_far h_turn_far_left h_turn_far_right
  have hnear : M.twice nearInscribed = config.nearArc.measure M :=
    inscribed_angle_from_triangle_turn G M config.nearArc.sense
      config.nearArc.start_onCircle config.nearArc.finish_onCircle
      config.farArc.finish_onCircle config.start_ne_opposite_far
      config.second_intersections_ne.symm h_turn_near_left h_turn_near_right
  have hnear_as_far_sense :
      nearInscribed =
        M.measure
          ⟨config.nearArc.finish, config.farArc.finish, config.nearArc.start,
            config.farArc.sense⟩ := by
    dsimp [nearInscribed]
    rw [config.senses_opposite]
    exact reverse_sense_measure G M config.farArc.sense
      config.start_ne_opposite_far
      config.second_intersections_ne
  have hnear_uses_vertex :
      nearInscribed =
        M.measure
          ⟨config.vertex, config.farArc.finish, config.nearArc.start,
            config.farArc.sense⟩ := by
    rw [hnear_as_far_sense]
    exact AngleMeasurement.Axioms.same_ray_invariant
      config.nearArc.finish config.vertex
      config.nearArc.start config.nearArc.start
      config.farArc.finish config.farArc.sense
      (sameRay_from_far_endpoint G config.second_secant
        config.second_intersections_ne)
      (sameRay_refl G config.start_ne_opposite_far)
  have hfar_split :
      farInscribed =
        M.add exterior nearInscribed := by
    dsimp [farInscribed, exterior]
    rw [hnear_uses_vertex]
    have h_triangle :
        M.measure
            ⟨config.farArc.start, config.nearArc.start, config.farArc.finish,
              config.farArc.sense⟩ =
          M.add
            (M.measure
              ⟨config.nearArc.start, config.vertex, config.farArc.finish,
                config.farArc.sense⟩)
            (M.measure
              ⟨config.vertex, config.farArc.finish, config.nearArc.start,
                config.farArc.sense⟩) := by
      apply exterior_angle_from_triangle_turn G M config.farArc.sense
        (v := config.vertex) (a := config.nearArc.start)
        (b := config.farArc.start) (d := config.farArc.finish)
        config.first_sameRay.1.symm config.first_intersections_ne.symm
        config.start_ne_opposite_far.symm config.second_sameRay.2.1.symm
        config.start_ne_opposite_far config.second_sameRay.2.1
        config.first_sameRay.1 config.first_secant
      exact h_turn_exterior
    rw [h_triangle]
    exact congrArg
      (fun t =>
        M.add t
      ...
      (AngleMeasurement.Axioms.same_ray_invariant
        config.nearArc.start config.farArc.start
        config.farArc.finish config.farArc.finish
        config.vertex config.farArc.sense
        config.first_sameRay
        (sameRay_refl G config.second_sameRay.2.1))
  apply eq_sub_of_add_eq G M
  calc
    M.add (M.twice exterior) (config.nearArc.measure M) =
        M.add (M.twice exterior) (M.twice nearInscribed) := by rw [hnear]
    _ = M.twice (M.add exterior nearInscribed) := (twice_add G M _ _).symm
    _ = M.twice farInscribed := congrArg M.twice hfar_split.symm
    _ = config.farArc.measure M := hfar
...
def Statement (G : Plane) (M : AngleMeasurement G) : Prop :=
  ∀ (circle : Circle G) (config : ExternalSecantConfiguration G circle),
    M.twice
        (M.measure
          ⟨config.farArc.start, config.vertex, config.farArc.finish, config.farArc.sense⟩) =
      M.sub (config.farArc.measure M) (config.nearArc.measure M)

/-- Sharygin, page 11, problem 4. -/
theorem problem4 (G : Plane) (M : AngleMeasurement G)
    [G.Axioms] [M.Axioms] :
    Statement G M := by
  intro circle config
  apply external_secant_from_triangle_turns G M config
  · exact triangle_turn G M config.farArc.sense
      (center_ne_onCircle G config.farArc.start_onCircle)
      (center_ne_onCircle G config.nearArc.start_onCircle)
      config.first_intersections_ne.symm
  · exact triangle_turn G M config.farArc.sense
      (center_ne_onCircle G config.nearArc.start_onCircle)
      (center_ne_onCircle G config.farArc.finish_onCircle)
      config.start_ne_opposite_far
  · exact triangle_turn G M config.nearArc.sense
      (center_ne_onCircle G config.nearArc.start_onCircle)
      (center_ne_onCircle G config.farArc.finish_onCircle)
      config.start_ne_opposite_far
  · exact triangle_turn G M config.nearArc.sense
      (center_ne_onCircle G config.farArc.finish_onCircle)
      (center_ne_onCircle G config.nearArc.finish_onCircle)
      config.second_intersections_ne.symm
  · exact triangle_turn G M config.farArc.sense
      config.first_sameRay.1.symm
      config.second_sameRay.2.1.symm
      config.start_ne_opposite_far

end Soultions.Sharygin.Page11.Problem4
